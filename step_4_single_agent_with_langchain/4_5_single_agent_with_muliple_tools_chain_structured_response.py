...
# Update the weather prompt to include format instructions
weather_prompt = ChatPromptTemplate.from_messages(
    [
        SystemMessagePromptTemplate.from_template(
            "You are a weather assistant. Return weather information in JSON format.\n{format_instructions}"
        ),
        HumanMessagePromptTemplate.from_template(
            "City: {city}\nProvide the weather details:"
        ),
    ]
)

# Update the weather chain with the parser
...
def get_weather(city: str) -> str:
    ...
    return str(result)  # Convert dict to string for the agent
